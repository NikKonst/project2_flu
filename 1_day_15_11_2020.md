...
`cat VarScan_results_alignment_1.vcf | awk 'FNR==1 {print "position,reference_base,alternative_base,frequency";next} NR>24 {gsub(",", ".", $10); split($10,a,":"); print $2, $4, $5, a[7]}' OFS=, > Lab\ notebooks/raw_data/VarScan_results_alignment_1.csv`

`cat VarScan_results_alignment_2.vcf | awk 'FNR==1 {print "position,reference_base,alternative_base,frequency";next} NR>24 {gsub(",", ".", $10); split($10,a,":"); print $2, $4, $5, a[7]}' OFS=, > Lab\ notebooks/raw_data/VarScan_results_alignment_2.csv`

`cat VarScan_results_alignment_3.vcf | awk 'FNR==1 {print "position,reference_base,alternative_base,frequency";next} NR>24 {gsub(",", ".", $10); split($10,a,":"); print $2, $4, $5, a[7]}' OFS=, > Lab\ notebooks/raw_data/VarScan_results_alignment_3.csv`

Results are available by [link](https://github.com/NikKonst/project2_flu/tree/master/raw_data).
